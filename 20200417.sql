SELECT 에서 연산 :
    날짜 연산(+, -) : 날짜 + 정수 : 날짜에서 +-정수를 한 과거 혹은 미래일자의 데이트 타입 반환
    정수 연산(....) : 수업시간에 다루진 않음
    문자열 연산
       리터럴 : 표기방법
               숫자 리터럴 : 숫자로 표현
...
문자열 매칭 연산 : LIKE 연산 / JAVA : .startsWith(prefix), .endWith(suffix)
마스킹 문자열 : % - 모든 문자열(공백 포함)
              _ - 어떤 문자열이든지 딱 하나의 문자        --잘 안씀
문자열의 일부가 맞으면 TRUE
...
SQL에서 NULL 값을 비교할 경우 일반적인 비교연산자(=)를 사용하지 못하고 IS 연산자를 사용
...
FROM emp
WHERE job IN ('SALESMAN')
   OR empno LIKE '78%'; --형변환 / 78%는 문자이기 때문에 형변환이 일어남. / 78, 780~789, 7800~7899
