...
총 4가지 제약조건이 존재
    ...
    CHECK : 컬럼에 입력되는 값을 개발자가 정한 로직에 의해 체크, 검증하는 제약
            ex : 성별 컬럼에 값은 F, M 두 개의 값만 들어갈 수 있도록 설정
/*
foreign key(참조키) : exerd에 있는 테이블끼리 연결해주는 선, 값이 잘못 들어가는 것을 방지, 많으면 많을수록 속도가 느려짐(검사량 많아져서)
not null은 check 제약에 포함됨 ==> 제약조건에 포함은 안되지만 사용을 많이해서 제약조건에 포함됨
*/

제약조건을 생성하는 방법
1. 테이블 생성 시 컬럼 옆에다가 해당 컬럼에 적용될 제약조건을 기술
   ==> 복합컬럼 설정이 불가
2. 테이블 생성시 컬럼 기술과 별도로해당 테이블에 적용될 제약조건을 기술
   ==> 복합컬럼 설정 가능 ***
