...
가능한 실행계획 조합 몇 개?? 굉장히 많다 ==> 짧은 시간 안에 해내야 한다(응답을 빨리 해야하므로)
...
JJE.v_emp ==> v_emp 시노님으로 생성
CREATE SYNONYM v_emp FOR JJE.v_emp;

v_emp를 통해 실제 객체인 JJE.v_emp를 사용할 수 있다.
...
SELECT *
FROM DBA_tables;    ---시스템 계정---

SELECT *
FROM dictionary;


Multiple insert : 여러 건의 데이터를 동시에 입력하는 insert의 확장구문
...
emp_test, emp_test2 테이블에 동시에 입력 ---테이블의 구조가 동일해서 동시 입력 가능
INSERT ALL
    INTO emp_test
    INTO emp_test2
...
2. conditional insert : 조건에 따라 입력할 테이블 결정
...
emp테이블에는 존재하고 emp_test 테이블에도 존재하는 직원을 이름 변경
...
emp테이블을 이용하여 부서번호별 직원의 급여 합과, 전체직원의 급여 합을 조회하기 위해
