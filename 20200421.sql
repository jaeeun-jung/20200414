...
SELECT empno, 5, 'test', LOWER('Hello, World')/*, UPPER('Hello, World'), INITCAP('hello, world') --임의로 지정한 상수같은 값 */
FROM emp; --행의 개수만큼 나옴
...
문자열 연산 함수
CONTCAT : 2개의 문자열을 입력으로 받아, 결합한 문자열을 반환한다.
...
SUBSTR(문자열, 시작인덱스, 종료인덱스) : 문자열의 시작인덱스 부터....종료인덱스 까지의 부분 문자열
...
SELECT SUBSTR('Hello, World', 1, 5) sub,
       ...
       INSTR('Hello, World', 'o', 6) ins2, --숫자는 시작하는 위치
...
SELECT MONTHS_BETWEEN(TO_DATE('2020/4/21', 'YYYY/MM/DD'), TO_DATE('2020/3/21', 'YYYY/MM/DD')),
       MONTHS_BETWEEN(TO_DATE('2020/4/22', 'YYYY/MM/DD'), TO_DATE('2020/3/21', 'YYYY/MM/DD'))
FROM dual;


ADD_MONTHS(DATE1, 가감할 개월수) : DATE1로부터 두번째 입력된 개월수만큼 가감한 DATE
오늘 날짜로부터 5개월 뒤 날짜
