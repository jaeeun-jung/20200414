...
PL/SQL, SQL을 사용하든 5라는 값을 게산해 내면 목적을 달성.
...
PRO4] cylce 테이블(애음요일)을 이용하여 daily(일실적) 테이블에 데이터를 생성해주는 프로시져
여사님 한 분이 관리하는 고객 2~300명
고객 한명당 제품을 2~3개 정도 애음
...
BEGIN
    --행의 결과는 여러 건인데 변수는 스칼라 변수 ==> 테이블 타입의 변수로 변경해야 한다.
...
만약에 해당하는 데이터가 없을 때 NO_DATA_FOUND
                ==> NO_EMP 개발자 정의 예외로 치환하여 의미상으로 좀 더 명확하게 표현
SELECT *
FROM emp
WHERE empno = :empno;
...
function2] 함수를 만들 때 어떤 인자가 필요한지,
           범용적으로 설계했을 때 어떤 현상이 발생하는지
