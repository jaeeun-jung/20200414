...
3. SELECT 결과를 INSERT
   ...
   ==> 여러 건의 데이터를 하나의 쿼리로 입력 가능(ONE-QUERY) ==> 성능 개선
...
INSERT INTO emp (empno, ename, hiredate) VALUES (9999, '정재은', SYSDATE);
...
empno 컬럼은 NOT NULL (DESC emp) - 에러
...
UPDATE, DELETE : 기존에 있는 걸 변경, 삭제
  == 쿼리를 작성할 경우 주의
     1. WHERE절이 누락되지 않았는지
     2. UPDATE, DELETE 문을 실행하기 전에 WHERE절을 복사해서 SELECT 하여
        영향이 가는 행을 눈으로 확인

서브쿼리를 이용한 데이터 수정
INSERT INTO emp (empno, ename, job) VALUES (9999, 'brown', NULL);

9999번 직원의 deptno, job 두 개의 컬럼을 SMITH 사원의 정보와 동일하게 변경
...
WHERE 절을 사용 가능한 DML : SELECT, UPDATE, DELETE
  ...
  데이터를 식별하는 속도에 따라서 쿼리의 실행 성능이 좌우된다.
  
INSERT : 빈 공간에 신규 데이터를 입력하는 것
         빈 공간을 식별하는게 중요
...
오라클에서는 첫번째 DML이 시작되면 트랜의 시작으로 인식
트랜잭션은 ROLLBACK, COMMIT을 통해 종료가 된다.

트랜잭션 종료 후 새로운 DML이 실행되면 새로운 트랜잭션이 시작된다.
...
즉 두개 중에 하나라도 문제가 생기면 일반적으로 ROLLBACK을 통해 두 개의 INSERT 구문을 취소.